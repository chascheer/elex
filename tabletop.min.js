...
function initializeTabletopObject( key, opts, filterActive, filterCol, filterName, sheetName, tableID )
{
  var dataTableInstance;
  var isSecondTable = !!sheetName;

  if ( !tableID ) { tableID = 'dataTable'; }
  ...
  Tabletop.init({
    ...
    callback: function( data, tabletop ) {
      if ( isSecondTable ) {
        ...
        dataTableInstance = writeTabletop( sheet.elements, opts, filterActive, filterName, tableID, dataTableInstance );
      } else {
        dataTableInstance = writeTabletop( data, opts, filterActive, filterName, tableID, dataTableInstance );
      }

      if ( filterActive ) {
        fillTabletopFilter( filterCol, tableID, dataTableInstance );
      }

    },
    simpleSheet: !isSecondTable,
    debug: false
  });
...
function writeTabletop( data, opts, filterActive, filterName, tableID, dataTableInstance )
{
  var options = $.extend( { data: data }, opts );

  $('#'+tableID).html('');

  if ( filterActive ) {
    $('#'+tableID).append( '<div class="form-inline" style="margin-bottom:0.5em;"><label for="'+tableID+'Filter">Filter by ' + filterName + ':&nbsp;</label><select id="'+tableID+'Filter" class="form-control"><option value="">(All)</option></select></div>' )
  }

  $('#'+tableID).append( '<table cellpadding="0" cellspacing="0" border="0" class="table table-condensed table-responsive" id="'+tableID+'Ext"></table>' );

  return $('#'+tableID+'Ext').DataTable( options );
}



function fillTabletopFilter( col, tableID, dataTableInstance )
{
  var select = $('#'+tableID+'Filter').on( 'change', function() {
    var val = $(this).val();

    dataTableInstance.column(col).search( val ? '^'+val+'$' : '', true, false ).draw();
  });

  dataTableInstance.column(col).data().unique().sort().each( function( d, j ) {
